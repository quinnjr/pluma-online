...
<svelte:head><title>{data.plugin.name} · Admin</title></svelte:head>
...
<header class="mb-6 flex flex-wrap items-start justify-between gap-4">
	...
	<form method="POST" action="?/delete" use:enhance onsubmit={(e) => { if (!confirm('Delete ' + p.name + '?')) e.preventDefault(); }}>
		<button type="submit" class="rounded-sm border border-rose-300 bg-rose-50 px-3 py-2 text-xs font-medium text-rose-800 hover:border-rose-500"><i class="fas fa-trash"></i> Delete plugin</button>
	</form>
</header>
...
<form method="POST" action="?/update" class="max-w-2xl space-y-5" use:enhance>
	...
	<div class="grid gap-5 sm:grid-cols-2">
		<div>
			<label class="font-sans text-xs font-semibold uppercase tracking-[0.14em] text-ink-500" for="categoryId">Category</label>
			<select id="categoryId" name="categoryId" required class="mt-2 w-full rounded-sm border-ink-300 bg-white px-3 py-2 text-sm">
				{#each data.categories as c (c.id)}<option value={c.id} selected={c.id === p.categoryId}>{c.name}</option>{/each}
			</select>
		</div>
		<div>
			<label class="font-sans text-xs font-semibold uppercase tracking-[0.14em] text-ink-500" for="languageId">Language</label>
			<select id="languageId" name="languageId" required class="mt-2 w-full rounded-sm border-ink-300 bg-white px-3 py-2 text-sm">
				{#each data.languages as l (l.id)}<option value={l.id} selected={l.id === p.languageId}>{l.name}</option>{/each}
			</select>
		</div>
	</div>
	<label class="flex items-center gap-2 text-sm"><input type="checkbox" name="verified" checked={p.verified} /> Verified plugin</label>
